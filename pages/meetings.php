...
echo "<!-- DEBUG: user_id = " . (isset($current_user['id']) ? $current_user['id'] : 'UNDEFINED') . " -->";
if (!isset($current_user) || !isset($current_user['id'])) {
    echo "<div style='color: red; padding: 20px; background: #ffebee; margin: 20px; border-radius: 8px;'>";
    echo "<h3>⚠️ ERRO DE AUTENTICAÇÃO</h3>";
    echo "<p>Usuário não está logado ou dados de sessão não foram carregados.</p>";
...
try {
    ...
    // SQL corrigido com todas as colunas necessárias
    $sql = "
        SELECT m.*, 
               u.name as creator_name,
               u.avatar as creator_avatar,
               0 as participant_count,
               0 as online_participants,
               CASE 
    ...
    ";
    $params = [isset($current_user['id']) ? $current_user['id'] : 0];
    
    /* SQL ORIGINAL (comentado para debug)
    $sql = "
        SELECT m.*, 
               u.name as creator_name,
               u.avatar as creator_avatar,
    ...
    */
    ...
    // DEBUG: Mostrar informações sobre as reuniões encontradas
    echo "<!-- DEBUG: SQL executado -->";
    echo "<!-- DEBUG: Parâmetros: " . print_r($params, true) . " -->";
    echo "<!-- DEBUG: Reuniões encontradas: " . count($meetings) . " -->";
    if (!empty($meetings)) {
        echo "<!-- DEBUG: Primeira reunião: " . print_r($meetings[0], true) . " -->";
    }
    ...
    <div class="meetings-list" id="meetings-list">
        ...
        <?php else: ?>
            <?php foreach ($meetings as $meeting): ?>
                <div class="meeting-card interactive-card" data-meeting-id="<?php echo $meeting['id']; ?>">
                    <div class="card-header">
                        <div class="meeting-header">
                            <h3 class="meeting-title gradient-title"><?php echo escape($meeting['title']); ?></h3>
                    ...
                    <div class="card-footer">
                        <div class="meeting-actions">
                            <?php if ($meeting['is_creator']): ?>
                                ...
                                <button class="btn btn-sm btn-outline text-error" onclick="meetingsModule.deleteMeeting(<?php echo $meeting['id']; ?>)">
    ...
    <div class="modal">
        ...
        <div class="modal-body">
            <form id="meeting-form" class="ajax-form" action="../api/meetings.php" method="POST">
                ...
                <!-- Campo de link removido pois a sala é interna -->
